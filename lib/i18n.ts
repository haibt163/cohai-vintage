...
export function localizedPost(post: EditorialPost, locale: Locale): EditorialPost {
  ...
  const vi: Record<string, Partial<EditorialPost>> = {
    ...
    "coco-chanel": { title: "Coco Chanel", category: "Lịch sử thời trang", excerpt: "Nhìn lại nhà thiết kế đã thay đổi mối quan hệ giữa vẻ thanh lịch, sự thoải mái và trang phục hiện đại.", intro: "Ảnh hưởng của Coco Chanel không chỉ nằm trong danh sách những món đồ nổi tiếng. Đó còn là cách nghĩ về việc một người phụ nữ chuyển động trong thế giới của mình với trang phục.", paragraphs: ["Ngôn ngữ thiết kế của Chanel thách thức những quy chuẩn gò bó của trang phục nữ đầu thế kỷ XX. Phom dáng đơn giản hơn, chất liệu thực dụng và bảng màu tiết chế giúp sự thoải mái trở thành một phần của từ điển thanh lịch.", "Bài học lâu bền của phong cách vintage nằm ở tỷ lệ và sự tiết chế. Một dáng hình dễ nhận biết vẫn có thể đáng nhớ mà không cần quá nhiều trang trí, trong khi một phụ kiện nhỏ có thể mang cá tính cho cả diện mạo.", "Những món vintage đặc biệt thú vị khi nhìn theo cách này. Điều đáng quan tâm không chỉ là một món đồ có cũ hay không, mà là liệu thiết kế của nó vẫn truyền tải được điều gì hữu ích, đẹp và rất riêng hay không."] },
  };
  return { ...post, ...vi[post.slug] };
}

export function localizedProduct(product: Product, locale: Locale): Product {
  if (locale === "en") return product;
  const vi: Record<string, Partial<Product>> = {
    "louis-vuitton-vintage-concorde": { name: "Louis Vuitton Concorde Vintage", category: "Túi xách Vintage", description: "Phom dáng Monogram vintage có cấu trúc, mang những tỷ lệ và cá tính gợi nhớ thiết kế Louis Vuitton thời kỳ du lịch kinh điển.", details: ["Louis Vuitton Monogram vintage", "Khoá S-lock", "Dây đeo điều chỉnh", "Sản phẩm vintage đã qua sử dụng"] },
    "louis-vuitton-neverfull-mm": { name: "Louis Vuitton Monogram Neverfull MM", category: "Túi xách Vintage", description: "Chiếc tote Monogram quen thuộc được nhìn qua lăng kính vintage và đồ đã qua sử dụng, nơi tình trạng và cá tính riêng của từng món đồ được đặt lên hàng đầu.", details: ["Canvas Louis Vuitton Monogram", "Kích thước MM", "Sản phẩm vintage đã qua sử dụng", "Tình trạng cần được xác nhận riêng"] },
    "louis-vuitton-vintage-mono-kelly": { name: "Louis Vuitton Mono Kelly Vintage", category: "Túi xách Vintage", description: "Một phom dáng vintage đặc trưng được tuyển chọn nhờ cấu trúc, phần cứng và lớp patina tạo nên cá tính riêng của hàng xa xỉ đã qua sử dụng.", details: ["Louis Vuitton Monogram", "Phom vintage có cấu trúc", "Phần cứng khoá đặc trưng", "Sản phẩm vintage đã qua sử dụng"] },
  };
  return { ...product, ...vi[product.slug] };
}
